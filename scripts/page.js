...
const repay = {
  ...
  init() {
    ...
    // 搜索按钮
    document.querySelector('#repay_search').addEventListener('click', () => {
      this.search(this);
    });
    ...
    // // 购买列表button
    $(document).on('click', '.repay_buyList', () => {
      location.href = './repaymentList';
    });

    // 选择按钮
    $(document).on('click', '.item-select', function () {
      $(this).toggleClass('jello').parents('li').toggleClass('current');
    });

    // // 还款button
    $(document).on('click', '.repay_toPay', e => {
      this.sureAlt('.alt_repay', e);
    });
    // 还款确定
    $(document).on('click', '.alt_repay .sure', this.sureRepay);

    // 短信通知
    $(document).on('click', '.repay_msg', e => {
      this.sureAlt('.alt_msg', e);
    });
    // 短信确定
    $(document).on('click', '.alt_msg .sure', this.sureMsg);

    // // 批量还款
    $(document).on('click', '#repay_all', () => {
      this.sureAlt('.alt_allRepay');
    });
  ...
  getData({ data, cb }) {
    ...
    // fetch('/ydwadmin.php/project/getRepaymentIndexList',{
    //   method:'GET',
    //   headers:myHeader,
    //   mode:'cors',
    //   cache:'default',
    ...
    // var myRequest = new Request('./getRepaymentIndexList.php', myInit);
  ...
  render(res) {
    ...
    if (Number(res.status) === 1 && res.data.length > 0) {
      ...
      Array.from(data, (item, index) => {
        listContent += `<li class=${item.status == 4 ? 'item_will' : 'item_has'} ><span><em class='animated zoomIn item-select'></em></span><span data-value=${item.id} class=item-project>${item.id}</span><span class=item-title data-value=${item.title}>${item.title}</span><span class=item-fid data-value=${item.fid}>${item.financing}</span><span>${item.due_time}</span><span>${item.amount}(元)</span><span>${item.total_ghost_amount}</span><span class=item-interest data-value=${item.total_due_interest}>${item.total_due_interest}</span><span class=item-capital data-value=${item.total_due_interest}>${item.total_due_capital}</span><span>${item.total_due_amount}</span><span>${item.status == 4 ? '还款中' : '已还款'}</span><span>${item.is_overdue ? '逾期' : '未逾期'}</span><span><i class=${item.status == 4 ? 'repay_toPay' : 'repay_msg'}>${item.status == 4 ? '还款' : '短信通知'}</i><i class=repay_buyList>购买列表</i></span></li>`;
      });
